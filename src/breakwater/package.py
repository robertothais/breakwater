...
app = typer.Typer(help="Manage Korean banking software packages.")
...
def list_packages():
    ...
    for target_key, target_info in manifest.items():
        name = target_info.get("name")
        target_branch = tree.add(f"{target_key}: {name}", style="bold")
        ...
        for arch, arch_info in target_info.items():
            ...
            packages = pm.list_downloaded_files(target_key)
...
def set_current(
    ...
    if not package.unpack_path.exists():
        typer.echo(
            f"Package not unpacked yet. Run: breakwater package unpack \
                {target} {architecture} --checksum {package.checksum}",
            err=True,
        )
        raise typer.Exit(1)
